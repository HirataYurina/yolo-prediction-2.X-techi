...
def arg():
    ...
    parser.add_argument('--num_classes', type=int, default=2, help='number of your classes')
    ...
    parser.add_argument('--weight_path', type=str, default='./data/ep180-loss20.398-best-helmet.h5',
                        help='the path of model weights')
    parser.add_argument('--save_path', type=str, default='./saved_model_helmet')
...
def save_model(inputs,
    ...
    raw_prediction_list = yolo3(inputs)
    for i, raw_prediction in enumerate(raw_prediction_list):
        ...
        # 1.decode
        pred_xy = ((tf.sigmoid(raw_xy) + anchor_xy[i]) / shape_float[::-1]) * input_hw[::-1]
